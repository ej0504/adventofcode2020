...
def star1_with_dict(target, list):
  dict = {};
  for n in list:
    if n in dict:
      return dict[n] * n;
...
def star2_with_dict(target, list):
  dict = {};
  for i in range(len(list)):
    for j in range(len(list)):  # O(n^2) to populate, allowing for + O(n) later to find. O(n^2) overall
      a = list[i];
      b = list[j];
      remainder = target - a - b;
      if i != j & remainder > 0:
        dict[remainder] = a * b;
...
print(star1_with_dict(2020, list));
print(star2_with_dict(2020, list));

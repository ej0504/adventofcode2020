...
def star1(target, list):
  list.sort();
  midway_index = find_midway_index(target, list);
  for i in range(midway_index):
    for j in range(midway_index, len(list)):
...
def star2():
  for n in list:
    subList = list[:];
    subList.remove(n);
    ans = star1(2020 - n, subList);
    if ans: 
      format(n, ans[0], ans[1]);
      return;


format_star1(star1(2020, list));
star2();
